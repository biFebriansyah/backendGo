...
import (
	...
	"github.com/biFebriansyah/backintro/database/orm/seeder"
...
func init() {
	SeedCmd.Flags().BoolVarP(&sedUp, "up", "u", true, "run seed up to insert data seed")
	SeedCmd.Flags().BoolVarP(&sedown, "down", "d", false, "run seed down to clear table")
}
...
func seedUp(db *gorm.DB) error {
	...
	for _, data := range seedModel {
		fmt.Println("create seeding data for", data.name)
...
func seedDown(db *gorm.DB) error {
	...
	for _, data := range seedModel {
		fmt.Println("delete seeding data for", data.name)
